...
| **X11 / Tcl-Tk** | Multiplayer SimCityNet on Unix workstations — ported to **Sun**, **SGI**, **NCD X terminal**, **DEC**, **HP**, **Linux**, and later **OLPC** | Same; **[collaborative-microworld-lineage.md](collaborative-microworld-lineage.md)** (constructionist / multiplayer context) · **[maxis-ea-shutdown-hn-2015.md](maxis-ea-shutdown-hn-2015.md)** §4 (DUX / trust thread) |
| **OLPC / Sugar**, GPL release | Educational fork, XO laptop; **PyGTK/Cairo/Pango** desktop UI | Jekyll pages **`apps/micropolis/website/pages/about/don-hopkins/olpc-simcity.md`**, **`building-simcity.md`**; external MOOLLM doc linked from root **`README.md`** (SimCity multiplayer & Micropolis) |
| **PyGTK desktop (revival candidate)** | Fully playable GTK UI from OLPC era — **not** current shipping stack | Legacy code under **`documentation/notes/legacy/`** (`PLAN.txt`, `OLPC-notes.txt`). **Forward path (Don, 2026):** PyGTK may still be viable (GTK ecosystem status TBD); preferred revival wires the **MicropolisCore C++ engine** to the Cairo/Pango UI shell — native desktop alongside Wasm+Svelte. See WWSFF **`simcity-lineage.yml`** (`pygtk_revival`, `native_ui_forward`). |
...
| **Wasm + SvelteKit + WebGPU/WebGL** | Current MicropolisCore app — **nested stack**: HTML/CSS/DOM + SVG/Canvas 2D + GPU holodeck compositor (WebGPU preferred; WebGL shipping) | **`apps/micropolis/`**, **`packages/render-core/`**, **`packages/tile-renderer/`**, **[unified-webgpu-renderer.md](unified-webgpu-renderer.md)**, **[renderer-plugin-roadmap.md](renderer-plugin-roadmap.md)**, **[wasm-bridge-and-testing-trajectory.md](wasm-bridge-and-testing-trajectory.md)**, **[micropolis-web-hn-2024.md](micropolis-web-hn-2024.md)** ([HN 40693944](https://news.ycombinator.com/item?id=40693944) launch) |
