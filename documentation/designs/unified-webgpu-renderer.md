...
## 7. Fallback raster (not the product GPU path)

**Product path:** the browser composes everything on the **client GPU** ([render-core-package.md §0](render-core-package.md#0-client-gpu-first-browser-composes-server-describes)). The server ships descriptions and stores approved uploads; it does not run the holodeck.

When WebGPU is unavailable:

- **Headless Chromium** for batch jobs should still prefer **WebGPU inside the browser process** when the worker has GPU access — same `HolodeckStage`, not a second server implementation.
- **`render/software`** + `MicropolisMapRenderDescription` produce RGBA on CPU for CI, Node-only pipelines, or explicit no-GPU export — no Cairo.

WebGL/Canvas scaffolds are teaching and compatibility only, not the Sims/Micropolis compositor.
