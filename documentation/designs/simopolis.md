# Simopolis: SimCity and The Sims Under One Umbrella
...
SimCity and The Sims were always the same game at different scales.

In SimCity, each **residential zone** is a colored tile representing thousands of anonymous people. In The Sims, each **person** has a name, a personality, 88 data fields, a family, relationships, a job, memories. Every SimCity zone is a compressed Sims neighborhood. Every Sims house is a SimCity lot.

**What if the compression was optional?**

A SimCity residential zone could open to reveal a full Sims neighborhood — families, relationships, motives — whose aggregate behavior feeds back into SimCity's simulation: tax revenue, crime rate, land value, commute demand. A Sims computer could display a running SimCity save, its traffic jams and budget crises as visible as the Sims' hunger bars. The two simulations are not just neighbors in a monorepo — they are **the same world at two resolutions**, and the player can zoom between them.
...
- **`packages/micropolis-engine`** — The Sims 1 simulation engine (C++/WASM), GPL-licensed
...
| VitaMoo character pipeline | Sims characters **and** future SimCity people at street level |
...
- SimAntics running a Lilliputian SimCity inside The Sims
...
- Sims characters can visit SimCity lots
- SimCity cities can contain Sims families
...
MicropolisCore/
  packages/
    micropolis-engine/     ← C++ sim → WASM (GPL)
  ...
  apps/
    micropolis/            ← SimCity SvelteKit app (current)
...
```
  ...
  apps/
    simopolis/             ← Unified SimCity + Sims shell (future)
...
### 3. SimCity zone ↔ Sims neighborhood link

Define the data contract: a SimCity lot position maps to a logical path under `content/micropolis/neighborhoods/`. The `sims-io` scanner reads Neighborhood.iff from that path. The Micropolis engine reads aggregate metrics back from the parsed family data. Small, well-defined interface.
...
| SimCity as constructionist microworld | [`collaborative-microworld-lineage.md`](collaborative-microworld-lineage.md) |
