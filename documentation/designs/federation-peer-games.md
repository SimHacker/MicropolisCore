...
| Aspect | Notes |
|---|---|
| Grade | 🟢🟢 — **spiritual fit** (pedagogy + town + coordination) looks strong from public materials; **technical fit pending** — no save-format bridge until documented and round-trip tested |
| Spiritual fit | Anti–instant-answer AI; problem-solving through curiosity, planning, creativity, collaboration; town + AI villagers — *may* align with Sims-lineage social texture and [legible social dynamics](https://github.com/SimHacker/moollm/blob/main/designs/legible-social-dynamics.md) **if** causes are systemic, not only chat |
...
### Why a bridge *might* matter someday (speculative — not beta scope)

**Only if** Sungman Cho wants character portability and publishes a lawful save format. The Federation’s unit of value is **identity**, not a screenshot: one soul-file; many **incarnations** ([characters-as-hydrogen.md](characters-as-hydrogen.md)) — e.g. a Sims `.iff` row, a MOOLLM citizen directory, and hypothetically a Sunny Street townsfolk slot.

**Hypothetical Sims → Sunny Street:** Bifrost read → soul-file → write into a Sunny Street save the player owns; Sunny Street’s engine still runs Sunny Street’s rules — no Sims embed, no EA asset redistribution.

**Hypothetical Sunny Street → Sims / Dream:** a villager a child invested in during play → soul-file → Pleasantview or Dream — **if** structured export exists.
...
**Near term:** compare notes on pedagogy and architecture (thinking-with-AI vs answer vending) — see outreach letter. **No ask** for the May 2026 closed beta.

**Long term (optional):** if Sunny Street wants interoperability, we offer an **open Bifrost target**: document the save shape, we ship a companion adapter in `packages/bifrost/adapters/`, the player owns both saves, both projects stay sovereign. Not a Micropolis mod, not a partnership claim.
...
**The house has room for exactly one.** One three-story house in cutaway, one man, one dog, one player, and no visitors — ever. And every disk held a *different* man: Activision duplicated LCP in-house and had the duplicator stamp an incrementing serial number on each disk, which seeded the name, shirt colour, personality and preferences, so the character was a property of the physical floppy rather than of the program. David Crane: *"Each disk was effectively unique."*

The missing dimension was designed and then dropped. Producer Sam Nelson on the sequel: *"a Little Computer People apartment complex. In that kind of a scenario, it would be more interesting to watch the interaction between the people, study the relationships that form, and so on. And you would be able to do things in several houses at once."* It was never released because *"Activision wasn't in the practice of sequelling what we had done already. So we just played around with it in the office for a little while and then dropped it on the floor"* ([Lemon64 interview archive](https://www.lemon64.com/doc/little-computer-people/353)). That sequel is what a bridge is: the interaction between people who each live on their own disk, arranged from outside the game, forty years late.
...
| Where the state lives | **256 bytes, at a known place on the disk** — on the C64 version, which is the one Crane programmed himself and the only one he documented. From his email to the Software Preservation Society: *"A 256 byte block of data on the disk held the LCPs 'brain' to keep personality status as the LCP aged and developed. If you have a disk that has been executed you will see that that block has been rewritten by the user's disk drive. If it is a virgin diskette that block will be in its initial state (probably all zeros)"* ([archived](https://web.archive.org/web/20250103095311/http://www.softpres.org/article:game:little_computer_people)). The smallest save file in the Federation, the serial number is its seed, and a played disk is distinguishable from a shrink-wrapped one by inspection. **The Apple ][ layout is unconfirmed** and is its own dig — probably the same idea, not necessarily the same offset. |
