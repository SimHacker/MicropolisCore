...
| T.10 | **Multi-streamer crossover sessions** | Two streamers each running Soul City with a household crossover; chats from both channels can vote jointly; the federated [Family Album graph](family-album-as-storymaker.md) records the crossover as a multi-author scene | The Bar-Karma writers'-room model with two writers' rooms; high-novelty, high-engagement, very shareable clips |
| T.11 | **"Twitch Plays Soul City" mode** | Extreme variant of T.1–T.4: chat directly drives the narrative loop without streamer approval, with rate-limited rounds and stronger safety filters. The 2014 *Twitch Plays Pokémon* moment, reproduced inside a household-sim. | Opt-in mode for events / charity streams / experiments; not the default |
...
| 2   | **Empty-state defensiveness** in character-card UI: never label a pre-author / zero state with a derived interpretive field; show blanks until the author has set values                                             | Soul City UI                                                 | 1 day                      |
