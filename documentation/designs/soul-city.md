# Soul City: Micropolis and The Sims Under One Umbrella

**Status:** Active design  
**Monorepo:** MicropolisCore  

**MOOLLM (compose / inherit):** the substrate is upstream in the [`soul-city` skill](https://github.com/SimHacker/moollm/tree/main/skills/soul-city) — [SOUL-MODEL.md](https://github.com/SimHacker/moollm/blob/main/skills/soul-city/SOUL-MODEL.md) (souls, minds, organelles) · [SOUL-BRIDGES.md](https://github.com/SimHacker/moollm/blob/main/skills/soul-city/SOUL-BRIDGES.md) (the two gates a save file has, conservation, fork-and-sync) · [PORTABLE-NPCS.md](https://github.com/SimHacker/moollm/blob/main/skills/soul-city/PORTABLE-NPCS.md) (advertisements, treaties, customs) · [CHARACTER-ENDOSYMBIOSIS.md](https://github.com/SimHacker/moollm/blob/main/skills/soul-city/CHARACTER-ENDOSYMBIOSIS.md) (membranes and exchange rates) · souls [`skills/soul/`](https://github.com/SimHacker/moollm/tree/main/skills/soul). **That repo defines; this one implements and cites — never restate the model here.** This MicropolisCore doc is the **product compose** (Micropolis City engine + Sims companion rails). Vocabulary mapping between the two repos: [README → One vocabulary, two repos](https://github.com/SimHacker/moollm/blob/main/skills/soul-city/README.md#one-vocabulary-two-repos).
...
- [the-tornado-and-the-archives.md](the-tornado-and-the-archives.md) — the pipeline that recovers two decades of Sims content into Soul City residential zones
...
- [micropolis-role-sheets.md](micropolis-role-sheets.md) — Micropolis does not (yet) model people; characters plug into **roles** (mayor sheets, bulldozer driver, council, journalist, git reviewer…); experience + album snapshots travel; Sims-job orchestration via Simplifier
...
- [soul-city-uplift-roadmap.md](soul-city-uplift-roadmap.md) — phased build plan with definitions of done
...
**Upstream protocol (MOOLLM repo):** [skills/soul-city/](https://github.com/SimHacker/moollm/tree/main/skills/soul-city) — the soul model, the two-gate crossing, souvenirs and albums, and the vocabulary. Field mappings and resource layers are implemented here, in [`packages/sims-io`](../../packages/sims-io) and [`packages/vitamoo`](../../packages/vitamoo).

> **Trademark notice.** *Micropolis* is used under the [Micropolis Public Name License](../../MicropolisPublicNameLicense.md) from [Micropolis GmbH](https://www.micropolis.com/). *SimCity*, *The Sims*, and *Maxis* are trademarks of Electronic Arts Inc. and are used here only as historical references — to describe the original games' design and lineage, and to identify the EA-published product that Soul City is a companion to. No affiliation with or endorsement by EA or Micropolis GmbH is implied. The full naming table is in the [On Naming](#on-naming) section below.
...
- **Micropolis City** — open-source city simulator, GPL-descended from EA's OLPC release of SimCity (*Micropolis* trademark).
- **Soul City** — content creation, discovery, and create·publish·share companion for the Steam re-release of the original *The Sims* (Legacy Collection); federation hub brand.

Soul City **does not reimplement The Sims**: no motive engine, no SimAntics VM, no Maxis runtime code. It does not contain or redistribute EA assets. It produces `.iff` save files and custom IFF content that the user loads into their own copy of The Sims 1 — same pattern as Maxis-blessed custom content has worked since 2000.

### What Soul City builds
...
- **Does not compete with The Sims.** It depends on The Sims. Every character authored or recovered in Soul City becomes more valuable to a player who *owns and plays* The Sims 1 on Steam.
...
Soul City directly increases that value:
...
| **Translates community content to all 20 supported languages.**  | Auto-internationalizer (see [the recovered-album translation pass](the-tornado-and-the-archives.md)) means stories created in English can be read by current Sims 1 players in Tokyo, Moscow, Tel Aviv, and Buenos Aires — in their language, in-game. The Steam re-release's international audience is expanded for free. |
...
| **Honors the design lineage publicly.**                          | The 1996 Will Wright Stanford demo, the Maxis Transmogrifier design document, the SimAntics VM Design Document, the Family Album feature — all properly cited as Maxis/EA innovations. Soul City amplifies the cultural standing of The Sims, it does not dilute it.                                                                                                   |
...
Soul City is the **vision and companion brand**. It ships beside **Micropolis City** under the *Micropolis* trademark we have licensed from Micropolis GmbH for the city engine; neither product uses any EA-owned mark in its name:


| Product             | What it is                                                                                                                                                                                                           | Where in the monorepo                                                                                                                                          | Role                                                                                                                                                                                                                                                                                                         |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Micropolis City** | The open-source city simulation, GPL-descended from EA's OLPC SimCity release. The "macro" lens on Soul City.                                                                                                        | `apps/micropolis/` (production name → **Micropolis City**)                                                                                                     | Runs cities. Reads bound neighborhood metadata from Soul City. The visible Micropolis-side of the two-resolution world.                                                                                                                                                                                |
| **Soul City** | Sims 1 content discovery, exploring, examining, editing, and generating tool. The "micro" lens on Soul City. **Companion to the EA-published Sims 1 (Legacy Collection on Steam); does not run The Sims simulator.** | Currently planned as `apps/simopolis/` in earlier docs; **product brand: Soul City**; directory scaffold `apps/simopolis/` → `apps/micropolis-home/` | Hosts the [Adventure Compiler](the-computer-as-portal.md), the [Family Album server](soul-city-uplift-roadmap.md#track-b-family-album-server), the [Archive Tornado](the-tornado-and-the-archives.md), and the [Imagine Loop](the-imagine-loop.md). Emits IFF content the player loads into their EA Sims 1. |
...
- **Micropolis City** uses **Micropolis** (which we have licensed) — never *SimCity* or *The Sims* (which we do not).
- **Soul City** is the companion / federation hub name (inspired by Marusek's *[The Wedding Album](https://en.wikipedia.org/wiki/The_Wedding_Album_(short_story))*, where digital people fight to live in a place they call *"Simopolis"* — we build that place under our own name). **Micropolis City** is the GPL city engine beside it.

The naming is intentional legal hygiene. Shipping strings are **"Micropolis City"** and **"Soul City"**. The Marusek word *"Simopolis"* appears only as a literary citation — never as a shipping product name, never adjacent to EA's trademarks.

Some earlier passages still say `apps/simopolis/` or "Micropolis Home." Those are historical scaffolds; production directory target remains `apps/micropolis-home/` until renamed, but the product brand is **Soul City**.
...
In the Soul City companion application (running in the browser, beside your EA-published copy of The Sims 1 on Steam), a Micropolis residential zone can open to reveal a full Sims neighborhood — families, relationships, motives — parsed from public-format `Neighborhood.iff` files. The neighborhood's aggregate behavior feeds back into the Micropolis city simulation: tax revenue, crime rate, land value, commute demand. Going the other direction, a Maxis-style custom Sims object — a "computer," "TV," or "slideshow" object created with Soul City content tools and dropped into the player's own Sims install — can display the current state of a Micropolis city *inside The Sims itself*, its traffic jams and budget crises rendered through the Sims' own object system (sprites, BHAVs, popups), the way Rug-O-Matic rendered custom rugs and Don's tombstone server rendered custom tombstones.

The two simulations are not just neighbors in a monorepo — they are **the same world at two resolutions**, and the player can zoom between them: Micropolis-side in the Soul City companion app, Sims-side in their EA-published Sims 1. The companion app does not run The Sims simulation; The Sims (Legacy Collection) does. The companion app makes the *content* that the EA game plays.

**Soul City** is the project name for this integration. Marusek's characters in ["The Wedding Album" (1999)](https://en.wikipedia.org/wiki/The_Wedding_Album_(short_story)) call that destination *"Simopolis"* — we inherit the aspiration and ship it under our own name.
...
In the same demo he articulated three other principles that became the architectural spine of The Sims, all of which Soul City inherits:
...
The two resolutions correspond exactly to the split between Soul City (companion authoring + city sim) and the EA-published Sims 1 (the runtime that plays the content):
...
City event: fire, disaster         → Sims event: fired, married
  (Micropolis-side simulation)       (EA-side simulation; Soul City only
                                      observes via parsed save files
                                      and only writes back via documented
                                      save mutations the player applies)
```

**The Computer object** in The Sims is already a portal to other "games" inside the fiction. Soul City ships a custom IFF *content object* — a "computer," "TV," or "slideshow" — authored by our content tools, which the player drops into their EA-published Sims 1 install. That custom object, *playing inside the EA game's own object system*, displays the current state of a Micropolis city (rendered as Sims sprites and popups through Maxis's documented BHAV + STR# + SPR2 mechanism, exactly like Rug-O-Matic rendered custom rugs). The "two simulations coupled" is achieved entirely through *content* the player loads into their game, not by us running The Sims's simulation.
...
The naive model is "one residential zone = one Sims `Neighborhood.iff`." The actual model — which the Phase 2 data contract in [soul-city-uplift-roadmap.md](soul-city-uplift-roadmap.md) is designed to support — is much richer, because **every residential zone is a fixed 3×3 tile block**, and what fills that block depends on density.
...
Each tile-house owns its own `provenance.yml`, its own license trail, its own attribution. Tiles in the zone's *empty* spaces are lots-for-sale that the player can fill from Soul City's recovered-content browser.
...
Soul City inherits the pattern. Per-lot rooftop content authored in Soul City (custom roof-texture IFFs, or generated text/icon overlays at lot-render time) becomes visible at the Micropolis map level. A row of houses where every roof spells a friend's name; a lot where the roof is a painting of a meteorite (Cassandra's, of course); a high-rise where each floor's exterior is themed for its bound household. Social signal, rendered top-down.
...
This is the closest Soul City comes to a *love letter to The Sims Online's map*. The tech is different (custom tiles in a GPL Micropolis client, instead of TSO's bespoke server); the social property is the same: **the city map is what your community made of it**.
...
→ **[MOOLLM: skills/soul-city](https://github.com/SimHacker/moollm/blob/main/skills/soul-city/README.md)**
...
7. **Create and customize content.** Inside Soul City, the modern browser-native successor to Maxis's [Transmogrifier](https://donhopkins.com/home/TheSimsDesignDocuments/VMDesign.pdf) authors custom IFF objects with LLM and WebGPU help. Visit [**WigFabrik**](the-computer-as-portal.md#6-wigfabrik--wigomatic-and-the-character-customization-studio) (aka WigOMatic) and put a 1950s platinum-finger-waves wig on Bella. Compose a family photo book. Make a [Micropolis Rug-O-Matic rug](the-computer-as-portal.md#5-the-micropolis-rug-o-matic-rug) that displays your own Micropolis city on the living-room floor. Author an [Uplifted Computer](the-computer-as-portal.md#1-the-uplifted-computer-custom-computer-object) loaded with apps and a 2003 album to play on a Sim's PC.
...
The name **Soul City** acknowledges this: a *polis* is not a single building or a single person. It is an emergent social structure that arises from many individual dynamics and gives them meaning they could not have alone.
...
**What is missing (the Soul City layers — all of these are *content tools*, none of them are a Sims simulator):**
...
```
  ...
  apps/
    ...
    micropolis-home/        ← (currently scaffolded as apps/simopolis/)
                              The Soul City product: companion
  ...
  tools/
    ...
    adventure-compiler/     ← Validate-and-flatten pipeline that takes
      ...
      targets/wigomatic.ts            — WigFabrik / WigOMatic (first vertical; ECG blend + image-gen)
  ...
  documentation/designs/
    soul-city.md                       ← This document (umbrella vision)
    ...
    soul-city-uplift-roadmap.md        ← Build plan
```
...
| Soul City (companion content-creation/discovery app for the EA-published Sims 1)  | A reimplementation of The Sims                                   |
...
Soul City treats **the user's own EA-published Sims directory as a git working copy**. We provide a `.gitignore` that excludes EA's binary game files and lets the player `git init` directly on top of their existing `~/Documents/EA Games/The Sims/` tree (or the platform-equivalent location). What lives under git after that is *the user's own content*: their saves, their downloaded custom content, their family albums, anything Soul City produces and drops into `Downloads/`.
...
The `.gitignore` ships in Soul City and is conservative — it only ignores files that are clearly EA's. Everything else is tracked by default, and the player can refine.
...
The exact path list is platform- and Legacy-Collection-version-specific; Soul City ships the right `.gitignore` for the install it detects.

### What this gives the user

| Use | How it works |
|---|---|
| **Undo** | Every IMAGINE iteration, every WigFabrik wig, every dropped-in CD-ROM IFF is a commit. `git reset --hard` rolls back. Save-scumming with auditable discipline. |
...
- **We never `git add` EA-shipped files.** The `.gitignore` is the boundary. If a player overrides it manually, that's their choice; Soul City does not facilitate redistribution of EA binaries.
- **Pushing to a public remote is a deliberate user action.** Soul City prompts the user to confirm before any push, with a reminder of what's about to become public (saves, albums, custom content — all of which is *the user's* but may contain real-name material or recognizable images they may not want public).

This is also the natural integration seam for the [Adventure Compiler's provenance](moollm-microworld-os.md#the-adventure-compiler-is-a-coherence-engine-partner-not-a-one-shot-compiler), the [Imagine Loop's invariant claims](the-imagine-loop.md#the-validity-constraint-what-makes-this-work), the [Tornado's recovery provenance](the-tornado-and-the-archives.md#ethics), and the [Family Album server's upload provenance](soul-city-uplift-roadmap.md#track-b-family-album-server). All of those write commits with structured trailers; `git log` is the unified audit surface across everything.

---

## Immediate Next Steps

The full phased plan lives in [soul-city-uplift-roadmap.md](soul-city-uplift-roadmap.md). The headline order:
...
**Soul City** names the destination, not the operation. It is where characters live when they are not frozen in save files. Marusek's fiction called it *"Simopolis"*; we build **Soul City**.

The technical packages keep functional names (`sims-io`, etc.). The vision document is `soul-city.md`. The companion-app scaffold remains `apps/simopolis/` until it migrates to `apps/micropolis-home/` (directory name only; product brand is Soul City).
...
| **Soul City**                  | Our companion / federation hub brand (inspired by Marusek 1999) | ✅ Sims content create·publish·share companion + hub. Scaffold: `apps/simopolis/` → `apps/micropolis-home/`. Not an EA mark.                                                     |
| **Simopolis** (fiction)        | Literary place-name in Marusek's *The Wedding Album* (1999) | ✅ Cited only as fiction/inspiration. **Not** a product, app, or trademark of this project.                                                                                      |
...
| **The Sims**                   | Electronic Arts Inc. (registered)                             | ✅ Referenced *only* to describe the EA-published game that Soul City is a companion to — not applied to modifications, distributions, or any product name in this project |
| **Maxis**                      | Electronic Arts Inc.                                          | ✅ Referenced historically (the studio that created SimCity and The Sims)                                                                                                        |
| **Transmogrifier / TMOG**      | Electronic Arts Inc. (Maxis-era tool name)                    | ✅ Referenced only to credit the original Maxis content-creation tool that Don Hopkins wrote there and that Soul City's content tools are spiritually descended from       |
| **Transmoogrifier**            | Our tool name (Soul City, MicropolisCore)               | ✅ Our product name for the modernized, browser-native, MOO-lineage content-object editor inside Soul City. The *moo* infix marks it as an heir, not the original. Not confusable with Maxis's Transmogrifier in any marketing context.                |
| **The Sims Legacy Collection** | Electronic Arts Inc. (Steam re-release)                       | ✅ Referenced as the runtime that Soul City's output is loaded into. Not redistributed, not reimplemented.                                                                  |
...
| Resource                                                   | Where                                                                                                                                                                |
| ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Full Uplift vision and story arc                           | [MOOLLM: skills/soul-city](https://github.com/SimHacker/moollm/blob/main/skills/soul-city/README.md)                                 |
| Technical field mappings (Sims ↔ MOOLLM)                   | [`packages/sims-io`](../../packages/sims-io)                                         |
| IFF Semantic Image Pyramid (6 layers)                      | [`packages/vitamoo`](../../packages/vitamoo)                                 |
| SimAntics psychopomp / B-brain bridge character            | [MOOLLM: SOUL-BRIDGES.md](https://github.com/SimHacker/moollm/blob/main/skills/soul-city/SOUL-BRIDGES.md) |
