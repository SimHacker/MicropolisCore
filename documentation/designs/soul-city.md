...
Soul City is the **vision**. The vision ships as **two complementary products**, both branded under the *Micropolis* trademark we have licensed from Micropolis GmbH, neither using any EA-owned mark in its product name:


| Product             | What it is                                                                                                                                                                                                           | Where in the monorepo                                                                                                                                          | Role                                                                                                                                                                                                                                                                                                         |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Micropolis City** | The open-source city simulation, GPL-descended from EA's OLPC SimCity release. The "macro" lens on Soul City.                                                                                                        | `apps/micropolis/` (production name → **Micropolis City**)                                                                                                     | Runs cities. Reads bound neighborhood metadata from Micropolis Home. The visible Micropolis-side of the two-resolution world.                                                                                                                                                                                |
...
- **Soul City** is the **umbrella / federation hub** name (inspired by Marusek's *[The Wedding Album](https://en.wikipedia.org/wiki/The_Wedding_Album_(short_story))*, where digital people fight to live in a place they call *"Simopolis"* — we build that place under our own name). The two Micropolis-branded products *under* the hub are **Micropolis City** and **Micropolis Home**.

The naming is intentional legal hygiene. When marketing materials, screenshots, app names, or external communication reference the shipping apps, the strings are **"Micropolis City"** and **"Micropolis Home"** (or **Soul City** for the federation hub / create·publish·share surface). The Marusek word *"Simopolis"* appears only as a literary citation — never as a shipping product name, never adjacent to EA's trademarks.

Some earlier passages still say `apps/simopolis/` or "Simopolis the app." Those are historical scaffolds for what is now planned as **Micropolis Home** in `apps/micropolis-home/` (and branded under the Soul City hub).
...
Soul City inherits the pattern. Per-lot rooftop content authored in Micropolis Home (custom roof-texture IFFs, or generated text/icon overlays at lot-render time) becomes visible at the Micropolis map level. A row of houses where every roof spells a friend's name; a lot where the roof is a painting of a meteorite (Cassandra's, of course); a high-rise where each floor's exterior is themed for its bound household. Social signal, rendered top-down.
...
The technical packages keep functional names (`sims-io`, etc.). The vision document is `soul-city.md`. The companion-app scaffold remains `apps/simopolis/` until it migrates to `apps/micropolis-home/` under the Soul City hub brand.
...
| **Soul City**                  | Our federation hub / umbrella vision name (inspired by Marusek 1999) | ✅ Vision + create·publish·share hub brand across design documents and catalogs. Not an EA mark.                                                                                  |
